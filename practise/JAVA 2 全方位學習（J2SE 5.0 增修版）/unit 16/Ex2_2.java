...
public class Ex2_2 extends WindowAdapter implements TextListener,ActionListener,AdjustmentListener
{
 Frame f;
 Dialog ddf;
 TextField tf;
 ...
 public Ex2_2()
 {
 	...
 	ddf=new Dialog(f,"設定顏色的對話盒",false);
 	...
 	MenuItem colorItem=new MenuItem("顏色對話盒");
 	colorItem.setActionCommand("14");
 	colorItem.addActionListener(this);
 	Menu color=new Menu("字型顏色設定");
 	color.add(colorItem);
 	
 	MenuShortcut exitMs=new MenuShortcut(KeyEvent.VK_X,false);
 	MenuItem exitItem=new MenuItem("結束程式",exitMs);
 	exitItem.setActionCommand("15");
 	exitItem.addActionListener(this);
 	Menu exitm=new Menu("檔案");
 	exitm.add(exitItem);
 ...
 public void actionPerformed(ActionEvent e)
 {
 	switch(Integer.parseInt(e.getActionCommand()))
 	{
 	 ...
 	 case 13:
 	   ...
 	   break;    		
 	 case 14:
 	   Panel pd=new Panel(new GridLayout(4,2));
 	   ...
 	   pd.add(new Label("紅色值 : "));
 	   pd.add(red);
 	   pd.add(new Label("綠色值 : "));
 	   pd.add(green);
 	   pd.add(new Label("藍色值 : "));
 	   pd.add(blue);
 	   ...
 	   ddf.add(pd);
  	 ddf.setSize(300,200);
 	   ddf.setLocation(200,200);
 	   ddf.addWindowListener(this);
 	   ddf.setVisible(true);
 	   break;   
 	 case 15:
 	   System.gc();
 ...
 {
 	int rred=red.getValue();
 	int rgreen=green.getValue();
 	int rblue=blue.getValue();
 	Color c=new Color(rred,rgreen,rblue);
 	if(cbfore.getState())
 	{
 	 lb.setForeground(c);
 	}
